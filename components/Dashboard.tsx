  ...
  return (
    <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto">
      <header className="mb-8">
        ...
        <p className="text-slate-500 mt-2">Continue sua jornada de expansão da consciência.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between h-40">
          <div>
            ...
            <p className="text-3xl font-bold text-slate-900 mt-2">{user.progress}%</p>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between h-40">
          <div>
            <p className="text-sm font-medium text-slate-500 uppercase tracking-wider">Pontuação Quantum</p>
            <div className="flex items-baseline gap-2 mt-2">
              <p className="text-3xl font-bold text-brand-600">{user.points}</p>
              <span className="text-sm text-slate-400">pts</span>
            </div>
          </div>
          <div className="text-sm text-slate-600">
            Próximo nível: <span className="font-semibold text-slate-900">3 (400 pts)</span>
          </div>
        </div>

        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between h-40">
          <div>
            <p className="text-sm font-medium text-slate-500 uppercase tracking-wider">Badges</p>
            <div className="flex -space-x-2 mt-3">
              {user.badges.map((badge, idx) => (
                <div key={idx} className="w-10 h-10 rounded-full bg-brand-100 border-2 border-white flex items-center justify-center text-xs font-bold text-brand-700" title={badge}>
                  {badge[0]}
                </div>
              ))}
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-slate-900">Em andamento</h2>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col md:flex-row">
          ...
          <div className="p-6 md:w-2/3 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="text-sm text-slate-500">{course.modules.length} Módulos</span>
                <span className="text-sm font-medium text-brand-600">Continuar de onde parou</span>
              </div>
              <h4 className="text-lg font-semibold text-slate-800 mb-2">Próxima Aula: A Dupla Fenda</h4>
      ...
      <section className="opacity-70 pointer-events-none grayscale">
        <h2 className="text-xl font-bold text-slate-900 mb-4">Materiais Complementares (Em Breve)</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="bg-slate-50 p-4 rounded-xl border border-slate-200 h-32 flex items-center justify-center">
