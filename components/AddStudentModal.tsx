...
interface AddStudentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddStudent: (studentData: { name: string; email: string; password: string; avatarUrl?: string }) => Promise<void>;
  isLoading: boolean;
}
...
export const AddStudentModal: React.FC<AddStudentModalProps> = ({ isOpen, onClose, onAddStudent, isLoading }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onAddStudent({ name, email, password, avatarUrl });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl p-6 shadow-2xl my-8 border border-slate-200 dark:border-slate-700 animate-scale-in">
        <h3 className="text-xl font-bold mb-6 text-slate-900 dark:text-white">Cadastrar Novo Aluno</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              required
            />
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">O aluno poderá alterar esta senha após o primeiro login.</p>
          </div>
          <div>
